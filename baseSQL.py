...
class InfoSQL:
    ...
    def insert_friend(self, name="", relationship="", id_card="", birth="", hometown="", education="", debt=0.0, note=""):
        insert_query = f"""
            INSERT INTO {self.table_name} (姓名, 关系, 身份证号, 生日, 家乡, 学历, 欠债, 备注) VALUES
            (?, ?, ?, ?, ?, ?, ?, ?)
        """
    ...
    def update_value(self, name: str, column: str, new_value):
        update_query = f"UPDATE {self.table_name} SET {column} = ? WHERE 姓名 = ?"
        data = (new_value, name)
    ...
    def select_friend(self, name):
        select_query = f"SELECT * FROM {self.table_name} WHERE 姓名 = ?"
    ...
    def findFriendByKeyword(self, keywords_list:list):
        ...
        for keyword in keywords_list:
            for column in ["姓名", "关系", "身份证号", "生日", "家乡", "学历", "欠债", "备注"]:  
                # 替换为实际的列名
